...
interface FakeGateway {
  ...
  readonly endWith: (n: number, failure?: SessionFailure) => Effect.Effect<boolean>
...
const makeStore = Effect.gen(function* () {
  ...
  return {
    ...
    clear: Ref.update(ref, (s) => ({ ...s, session: null, clears: s.clears + 1 })),
    inspect: Ref.get(ref),
  }
})
...
const makeGateway = Effect.sync((): FakeGateway => {
  ...
  return {
    ...
    endWith: (n, failure) =>
      Effect.suspend(() =>
        failure === undefined ? Deferred.succeed(sessions[n]!.done, void 0) : Deferred.fail(sessions[n]!.done, failure),
      ),
    crash: (n) => Effect.suspend(() => Deferred.die(sessions[n]!.done, new Error('boom'))),
  }
})
...
const waitForSessions = (gateway: FakeGateway, n: number) => waitFor(Effect.map(gateway.count, (c) => c >= n))
...
it.effect('cold boot identifies freshly; graceful shutdown keeps the session for a later resume', () =>
  Effect.gen(function* () {
...
it.effect('resumes from persisted session and persists sequence advances', () =>
  Effect.gen(function* () {
    const gateway = yield* makeGateway
    const store = yield* makeStore
    ...
    expect((yield* store.inspect).saves.slice(-1)).toEqual([{ sessionId: 's9', sequence: 43 }])
...
it.effect('terminal close stops supervision permanently and clears the session', () =>
  Effect.gen(function* () {
    const gateway = yield* makeGateway
    const store = yield* makeStore
...
it.effect('non-terminal disconnect retries with exponential capped full-jitter backoff (TestClock)', () =>
  Effect.gen(function* () {
    const gateway = yield* makeGateway
    const store = yield* makeStore
