...
export default packageJson({
  ...
  scripts: {
    ...
    'cf:state:migrate:dry-run':
      'node --experimental-strip-types cf/src/deploy-preflight.ts && (cd cf && node --experimental-strip-types scripts/state-migrate.ts --dry-run)',
    'cf:state:migrate:execute':
      'node --experimental-strip-types cf/src/deploy-preflight.ts && (cd cf && node --experimental-strip-types scripts/state-migrate.ts --execute)',
    'cf:state:verify-equal':
      'node --experimental-strip-types cf/src/deploy-preflight.ts && (cd cf && node --experimental-strip-types scripts/state-migrate.ts --verify-equal)',
    'cf:state:verify-remote-authoritative':
      'node --experimental-strip-types cf/src/deploy-preflight.ts && (cd cf && node --experimental-strip-types scripts/state-migrate.ts --verify-remote-authoritative)',
    'e2e:live': 'node --experimental-strip-types e2e/src/live-main.ts',
    start: 'node --experimental-strip-types src/main.ts',
    test: 'vitest run',
    'test:e2e': 'vitest run e2e/src --no-file-parallelism',
