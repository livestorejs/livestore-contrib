    ...
    expect(desiredApplicationCommands).toEqual([
      {
        type: 1,
        name: "docs",
        description: "Ask LiveStore docs via OpenAI (store:false); no ambient chat or bot-retained query/answer content.",
        options: [{
          type: 3,
          name: "query",
...
describe("diffApplicationCommands", () => {
  ...
  it("reports create, update, and delete without hiding bulk-replacement impact", () => {
    const desired: ReadonlyArray<ApplicationCommand> = [
      command(1, "alpha", "new"),
      command(3, "Create Thread", ""),
    ]
    const actual: ReadonlyArray<ApplicationCommand> = [
      command(1, "alpha", "old"),
      command(1, "obsolete", "remove me"),
    ]

    expect(diffApplicationCommands(desired, actual).changes.map(({ kind, key }) => ({ kind, key })))
      .toEqual([
  ...
  it("detects global installation and interaction-context drift as unordered sets", () => {
    ...
    expect(diffApplicationCommands([desired], [{ ...desired, integrationTypes: [1, 0], contexts: [1, 0] }]).hasChanges)
      .toBe(false)
    expect(diffApplicationCommands([desired], [{ ...desired, integrationTypes: [0], contexts: [0] }]).hasChanges)
      .toBe(true)
  })
  ...
  it("detects every supported string-option constraint", () => {
    ...
    for (const actualOption of actualOptions) {
      expect(diffApplicationCommands([constrained], [{
        ...constrained,
        options: [actualOption],
      }]).hasChanges).toBe(true)
    }
  })
})
