...
describe('DFX application command port', () => {
  it.effect('uses only the configured guild route for staging-style scope', () =>
    Effect.gen(function* () {
      ...
      expect(calls).toEqual([`list-guild:${applicationId}:${guildId}`, `replace-guild:${applicationId}:${guildId}`])
  ...
  it.effect('makes global guild-only visibility explicit', () =>
    Effect.gen(function* () {
      ...
      expect(replaced.every((command) => command.contexts?.[0] === 0 && command.integrationTypes?.[0] === 0)).toBe(true)
    }),
  )
...
const fakeRest = (
  ...
  listGuildApplicationCommands: (appId, targetGuildId) =>
    Effect.sync(() => {
  ...
  bulkSetGuildApplicationCommands: (appId, targetGuildId, requests) =>
    Effect.sync(() => {
...
const responseFromRequest = (
  ...
  default_member_permissions:
    request.default_member_permissions === undefined
      ? null
