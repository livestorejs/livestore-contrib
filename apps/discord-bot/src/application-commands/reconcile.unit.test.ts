...
describe("application command reconciler", () => {
  it.effect("keeps diff read-only", () =>
    Effect.gen(function* () {
      let replaces = 0
      const reconciler = makeApplicationCommandsReconciler(port({
        actual: [],
        onReplace: () => replaces++,
      }))
  ...
  it.effect("refuses to replace an invalid duplicate inventory", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* makeApplicationCommandsReconciler(port({
        actual: [duplicate, duplicate],
        onReplace: () => replaces++,
      })).sync(scope).pipe(Effect.exit)
  ...
  it.effect("fails when Discord returns residual drift after replacement", () =>
    Effect.gen(function* () {
      let replaces = 0
      const error = yield* makeApplicationCommandsReconciler(port({
        actual: [],
        afterReplace: [{ ...desiredApplicationCommands[0], description: "still stale" }],
        onReplace: () => replaces++,
      })).sync(scope).pipe(Effect.flip)
...
const port = ({
  ...
  replace: (_scope, commands) => Effect.sync(() => {
    onReplace()
