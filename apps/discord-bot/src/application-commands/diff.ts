...
export const diffApplicationCommands = (
  ...
  return {
    ...
    duplicateActualKeys,
    hasChanges:
      duplicateActualKeys.length > 0 || changes.some(change => change.kind !== "unchanged"),
  }
}
  ...
  left.options.every((option, index) => {
    ...
    return other !== undefined &&
      ...
      (option.choices ?? []).every((choice, choiceIndex) => {
        ...
        return otherChoice !== undefined &&
          choice.name === otherChoice.name &&
          choice.value === otherChoice.value
      })
  })
