...
export const makeDfxApplicationCommandsPort = (rest: ApplicationCommandsRest): ApplicationCommandsPort => ({
  list: (scope) => {
...
const fromDiscordCommand = (
  ...
  return Effect.succeed({
    ...
    ...(command.integration_types === undefined ? {} : { integrationTypes: [...command.integration_types] }),
...
const unsupported = (commandKey: string, detail: string) =>
  Effect.fail(
    new ApplicationCommandRemoteShapeUnsupported({
      commandKey,
      message: `Discord returned ${detail}; refusing to reconcile a lossy command projection`,
    }),
  )

const mapRestError = (operation: ApplicationCommandRestError['operation'], scope: ApplicationCommandScope) =>
  Effect.mapError(
    (cause: unknown) =>
