...
export const makeLocalBotControl = (options: {
  ...
  const create = (input: {
    ...
    return options.sourceReader.read(input.source).pipe(
      Effect.mapError(error => error.kind === "unavailable"
      ...
      Effect.flatMap(facts => options.sourceObserver.observeSourceThread({
        sourceMessageId: input.source.messageId,
        channelId: input.source.channelId,
      }).pipe(
        Effect.mapError(() => new ControlDependencyUnavailable({
          dependency: "discord-thread-observation",
          message: "Existing source thread could not be authoritatively checked",
        })),
        Effect.flatMap(observation => observation._tag === "Unrun"
          ? Effect.fail(new ControlDependencyUnavailable({
              dependency: "discord-thread-observation",
              message: "Existing source thread could not be authoritatively checked",
            }))
          : options.thread(candidateForOperator(
              input.source,
  ...
  return { invoke: (operation, payload, principal) => {
    try {
      ...
      switch (operation) {
          ...
          return options.sourceReader.read(input.source).pipe(
            ...
            Effect.flatMap(facts => options.sourceObserver.observeSourceThread({
              ...
              Effect.flatMap(observation => {
                if (observation._tag === "Unrun") return Effect.fail(new ControlDependencyUnavailable({
                  dependency: "discord-thread-observation",
                  message: "Existing source thread could not be authoritatively checked",
                }))
        ...
        case "DocsQuery": {
          const input = Schema.decodeUnknownSync(Schema.Struct({ query: Schema.String, refreshCorpus: Schema.Boolean }))(payload)
          return options.docs.query({ surface: "cli", query: input.query, refreshCorpus: input.refreshCorpus }).pipe(
            Effect.map(result => success(renderDocsResult(result))),
          )
        }
        case "DocsStatus": return Effect.succeed(success("Documentation workflow is configured and readiness-admitted."))
        case "RuntimeHealth": return Ref.get(options.health).pipe(Effect.map(state => success(
        ...
        case "RuntimeStatus": return Ref.get(options.health).pipe(Effect.map(state => success(
          `environment=${options.config.environment} mode=${options.config._tag} state=${state.state}`,
        )))
        case "ConfigValidate": {
        ...
        case "EffectiveConfig": return Effect.succeed(success(JSON.stringify(summarizeConfig(options.config))))
        case "AuthStatus": return Effect.succeed(success(
          `principal=${principal.id} provenance=${principal.provenance} read=${principal.canRead} write=${principal.canWrite}`,
        ))
        case "ApplicationCommandsDiff": return options.commands.diff(options.config.commandScope).pipe(
        ...
        case "StagingE2ERun": return unsupported("Use the dedicated staging E2E harness; this runtime does not self-authorize live writes.")
      }
    } catch {
...
export const serveBotControl = (
  ...
  Effect.tryPromise({
    ...
    catch: cause => new ControlDependencyUnavailable({
  ...
  server => Effect.promise(() => new Promise<void>(resolve => server.close(() => resolve())))
    .pipe(Effect.andThen(Effect.promise(() => unlink(path).catch(() => undefined)))),
).pipe(Effect.withSpan("runtime.control.serve"))

/** A typed client facade over a one-request-per-connection versioned protocol. */
export const makeUnixBotControlClient = (path: string) => Effect.gen(function* () {
  ...
  const built = yield* RpcClient.makeNoSerialization(BotControl, {
    ...
    onFromClient: ({ message }) => {
      ...
      return Effect.exit(sendRequest(path, message.tag, message.payload)).pipe(
        Effect.flatMap(exit => writeResponse === undefined
...
const sendRequest = (path: string, operation: BotControlOperation, payload: unknown) => Effect.callback<ControlResultType, ControlErrorType>(resume => {
  ...
  socket.once("error", () => resume(Effect.fail(new ControlDependencyUnavailable({ dependency: "control-socket", message: "Could not connect to bot control socket" }))))
  socket.once("end", () => {
    try {
      ...
      resume(Effect.fail(new ControlDependencyUnavailable({ dependency: "control-socket", message: "Bot control response was invalid" })))
    }
  })
  ...
  socket.once("end", () => {
    ...
    Effect.runPromiseExit(
      resolveControlPrincipal(socket, path, policy).pipe(
        Effect.flatMap(principal => auditControlOperation(
          operation,
          request.payload,
          principal,
    ...
    }).catch(() => socket.end(`${encodeResponse({
      apiVersion: 1,
      id: request.id,
...
const operationNames = new Set<string>([
  "ThreadInspect", "ThreadPlan", "ThreadCreate", "ThreadStatus", "ThreadReconcile", "ThreadPolicyExplain",
  "DocsQuery", "DocsStatus", "RuntimeHealth", "RuntimeStatus", "ConfigValidate", "EffectiveConfig", "AuthStatus",
  "ApplicationCommandsDiff", "ApplicationCommandsSync", "StagingE2ERun",
])
...
const isWriteOperation = (operation: BotControlOperation, payload: unknown): boolean => {
  ...
  return operation === "ThreadReconcile" && typeof payload === "object" && payload !== null &&
    "apply" in payload && payload.apply === true
}
...
const auditControlOperation = (
  ...
  return effect.pipe(
    Effect.tap(result => Effect.logInfo("Control write completed").pipe(
      Effect.annotateLogs({ controlOutcome: result._tag }),
      annotate,
    )),
    Effect.tapError(error => Effect.logWarning("Control write rejected or failed").pipe(
...
const controlResultFromThreadOutcome = (outcome: ThreadOutcome): Effect.Effect<ControlResultType, ControlErrorType> => {
  switch (outcome._tag) {
    case "Created": return Effect.succeed(success(`Created thread ${outcome.threadId}.`, "Success", outcome.source.messageId))
    case "AlreadySatisfied": return Effect.succeed(success(`Thread ${outcome.threadId} already satisfies the request.`, "AlreadySatisfied", outcome.source.messageId))
    case "AuthorizationRejected": return Effect.fail(new ControlApplicationFailure({ message: "Thread request was not authorized" }))
    case "PolicyRejected": return Effect.fail(new ControlApplicationFailure({ message: `Thread request rejected by policy: ${outcome.reason}` }))
    case "TerminalFailure": return Effect.fail(new ControlApplicationFailure({ message: `Thread creation failed: ${outcome.failureCode}` }))
    case "TransientFailure": return Effect.fail(new ControlAmbiguousOutcome({ message: `Thread outcome requires reconciliation: ${outcome.failureCode}`, correlationId: outcome.source.messageId }))
  }
}
...
const renderReconciliationResult = (applied: boolean) => (result: ReconciliationResult): ControlResultType => {
...
const commandDiffResult = (
  diff: { readonly changes: ReadonlyArray<{ readonly kind: "create" | "update" | "delete" | "unchanged" }>; readonly hasChanges: boolean },
...
const readPeerCredentials = (socket: Socket): { readonly uid: number; readonly gid: number; readonly pid: number } | undefined => {
...
const socketAcceptsConnections = (path: string) => new Promise<boolean>(resolve => {
  const socket = createConnection(path)
  socket.once("connect", () => {
    socket.destroy()
    resolve(true)
