...
import { canonicalizeRuntimeConfig, RuntimeConfigFile, RuntimeConfigError } from './config-schema.ts'
...
export const loadRuntimeConfig = Effect.fn('runtime.config.load')(function* (path: string) {
  ...
  return yield* Schema.decodeEffect(Schema.fromJsonString(RuntimeConfigFile), { onExcessProperty: 'error' })(text).pipe(
