...
describe('deployable runtime tracer bullet', () => {
  it.effect('crosses Unix RPC, shared workflow, SQLite, fake Discord, docs, and readiness', () =>
    Effect.gen(function* () {
      ...
      const created = yield* client.ThreadCreate({
        source,
        environment: 'staging',
        apply: true,
        reason,
      ...
      const duplicate = yield* client.ThreadCreate({
        source,
        environment: 'staging',
        apply: true,
        reason,
...
const testConfig = Effect.gen(function* () {
  ...
  yield* Effect.addFinalizer(() =>
    fileSystem.remove(stateDirectory, { recursive: true, force: true }).pipe(Effect.orDie),
  )
  return {
    _tag: 'fake',
    environment: 'staging',
