...
export const FakeDiscordActionsLive = Layer.succeed(DiscordActions, DiscordActions.of({
...
export const FakeDocsPortsLive = Layer.mergeAll(
  Layer.effect(DocumentationCorpus, Effect.gen(function* () {
  ...
  Layer.succeed(AnswerEngine, AnswerEngine.of({
    ...
    answer: input => Effect.succeed({
      candidate: { supported: true, answer: `Fake source-backed answer for: ${input.query}`, citations: [fakeSource.id] },
