...
describe("Discord docs delivery", () => {
  it.effect("edits the deferred public response then sends ordered public follow-ups", () => Effect.gen(function* () {
    ...
    expect(delivered.map(({ kind, message }) => ({ kind, content: message.content, visibility: message.visibility }))).toEqual([
  ...
  it.effect("stops ordered delivery after a follow-up failure", () => Effect.gen(function* () {
    const delivered: Array<{ readonly kind: string; readonly message: InteractionMessage }> = []
    let followUpOrdinal = 0
    const actions: DiscordActionsService = {
      ...recordingActions(delivered),
      followUpInteractionResponse: message => Effect.suspend(() => {
        followUpOrdinal += 1
...
const recordingActions = (
  ...
  editInteractionResponse: message => Effect.sync(() => { delivered.push({ kind: "edit", message }) }),
  followUpInteractionResponse: message => Effect.sync(() => { delivered.push({ kind: "follow-up", message }) }),
