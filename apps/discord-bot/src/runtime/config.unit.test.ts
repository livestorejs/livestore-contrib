...
describe('runtime config', () => {
  ...
  it('rejects embedded credential values and unknown versions', () => {
    expect(() =>
      Schema.decodeUnknownSync(RuntimeConfigFile, { onExcessProperty: 'error' })({
        apiVersion: 1,
        payload: {
          _tag: 'real',
          ...base,
          credentials: { discordTokenFile: '/run/x', openAiApiKeyFile: '/run/y', discordToken: 'secret' },
        },
      }),
    ).toThrow()
    expect(() =>
      Schema.decodeUnknownSync(RuntimeConfigFile)({ apiVersion: 2, payload: { _tag: 'fake', ...base } }),
    ).toThrow()
  })
    ...
    expect(() =>
      Schema.decodeUnknownSync(RuntimeConfigFile)({
        ...
        payload: {
          _tag: 'fake',
          ...base,
          applicationId: '1310646763505582171',
          commandScope: { _tag: 'GlobalCommandScope', applicationId: '1310646763505582171' },
        },
      }),
    ).toThrow()
    expect(() =>
      Schema.decodeUnknownSync(RuntimeConfigFile)({
        ...
        payload: {
          _tag: 'fake',
          ...base,
          commandScope: { _tag: 'GlobalCommandScope', applicationId: base.applicationId },
        },
      }),
    ).toThrow()
  })
    ...
    expect(() =>
      Schema.decodeUnknownSync(RuntimeConfigFile)({
        ...
        payload: {
          ...
          docsAudience: {
            publicChannelIds: base.docsAudience.publicChannelIds,
            roleRestrictedChannelIds: ['100000000000000003'],
            contributorMaintainerRoleIds: ['100000000000000004'],
          },
        },
      }),
  ...
  it('classifies only decoded Discord 4xx responses as definitive mutations', () => {
    expect(
      isDefinitiveDiscordMutationFailure({
    ...
    expect(isDefinitiveDiscordMutationFailure({ _tag: 'HttpClientError', reason: { _tag: 'TransportError' } })).toBe(
      false,
    )
  })
  ...
  it('fails closed when the token application identity differs', async () => {
    await expect(
      Effect.runPromise(
        verifyDiscordApplicationIdentity(
          {
    ...
    ).rejects.toMatchObject({ _tag: 'DiscordIdentityAdmissionError' })
  })

  it('admits readiness only after an actual READY dispatch', async () => {
    await expect(
      Effect.runPromise(awaitGatewayReadySignal(Stream.make({ _tag: 'Ready', shardId: 0 }), Effect.never)),
    ).resolves.toBeUndefined()
    for (const malformed of [null, {}, { _tag: 'Ready', shardId: Number.NaN }]) {
      await expect(
        Effect.runPromise(awaitGatewayReadySignal(Stream.make(malformed), Effect.never)),
      ).rejects.toMatchObject({ _tag: 'GatewayReadinessError', reason: 'stream_ended' })
    }
    await expect(Effect.runPromise(awaitGatewayReadySignal(Stream.empty, Effect.never))).rejects.toMatchObject({
      _tag: 'GatewayReadinessError',
      reason: 'stream_ended',
    })
    await expect(
      Effect.runPromise(
        awaitGatewayReadySignal(
          Stream.never,
          Effect.fail(new GatewayReadinessError({ reason: 'terminal_failure', message: 'terminal_gateway_failure' })),
        ),
      ),
