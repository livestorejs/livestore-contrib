...
export const makeDocsWorkflowLayer = (admissionOptions: DocsWorkflowOptions = {}) => Layer.effect(
  DocsWorkflow,
  Effect.gen(function* () {
    ...
    const query = Effect.fn('docs.workflow.query')(function* (input: DocsQueryInput) {
      ...
      const estimatedInputTokens = estimateOpenAiRequestTokenUpperBound({ query: normalizedQuery, corpusDigest: snapshot.digest, sources })
      const monthlyReservation = admissionOptions.stateStore !== undefined && admissionOptions.monthlyCostUsdMicros !== undefined
        ? yield* admissionOptions.stateStore.reserveMonthly({
          atMillis: Date.now(),
          costUsdMicros: admissionOptions.estimatedCostUsdMicros?.({
      ...
      if (monthlyReservation?._tag === 'Denied') {
        yield* telemetry.emit(event(input, 'admission_denied', snapshot, cacheStatus, engine.configurationIdentity, sources.length, undefined, 'monthly_cost'))
      ...
      const generated = yield* engine
        ...
        .pipe(
          Effect.tap(({ usage }) => Effect.sync(() => { observedUsage = usage })),
          Effect.result,
      ...
      if (admissionOptions.stateStore !== undefined) {
        const correlation = admissionOptions.correlationKey === undefined
          ? createHash('sha256').update(`${input.surface}:${snapshot.digest}:${engine.configurationIdentity}:${Date.now()}`).digest('hex')
          : correlateWithKey(admissionOptions.correlationKey, `${input.surface}:${snapshot.digest}:${engine.configurationIdentity}:${Date.now()}`)
        yield* admissionOptions.stateStore.record({
          ...
          quota: {
            principal: admissionOptions.correlationKey === undefined
              ? createHash('sha256').update(input.principalId ?? input.surface).digest('hex')
