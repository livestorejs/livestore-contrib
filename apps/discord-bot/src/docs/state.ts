...
const StateFile = Schema.Struct({
  ...
  monthly: Schema.Array(Schema.Struct({
    id: Schema.String,
    atMillis: NonNegativeInt,
    costUsdMicros: NonNegativeInt,
...
export interface DocsStateStore {
  readonly record: (input: { readonly provenance: AnswerProvenance; readonly quota: DocsQuotaSample }) => Effect.Effect<void>
  ...
  readonly reserveMonthly: (input: { readonly atMillis: number; readonly costUsdMicros: number; readonly ceilingUsdMicros: number }) => Effect.Effect<MonthlyReservation>
  /** Settles a reservation; unknown provider usage deliberately keeps its reservation. */
  readonly settleMonthly: (input: { readonly id: string; readonly outcome: 'cancel' | 'charge'; readonly costUsdMicros?: number }) => Effect.Effect<void>
}
...
export const makeFileDocsTelemetry = (stateDirectory: string) => ({
  emit: (event: DocsTelemetryEvent) => Effect.logDebug("docs telemetry").pipe(
    Effect.annotateLogs({ ...event, diagnosticDirectory: stateDirectory }),
  ),
})
...
export const makeFileDocsStateStore = (stateDirectory: string, now: () => number = Date.now): DocsStateStore => {
  ...
  const load = async (): Promise<StateFile> => {
    try {
      ...
      if (typeof error === "object" && error !== null && "code" in error && error.code === "ENOENT") return { version: 1, provenance: [], quota: [], monthly: [] }
      throw error
    }
  }
  ...
  return {
    record: input => Effect.tryPromise(() => serialized(path, async () => {
      const at = now()
    ...
    monthlySpent: nowMillis => Effect.tryPromise(() => serialized(path, async () => {
    ...
    reserveMonthly: input => Effect.tryPromise(() => serialized(path, async () => {
      ...
      await persist({
        ...value,
        monthly: [...value.monthly, { id, atMillis: input.atMillis, costUsdMicros: input.costUsdMicros, status: 'reserved' }],
      })
      return { _tag: 'Reserved', id }
    })),
    settleMonthly: input => Effect.tryPromise(() => serialized(path, async () => {
      ...
      monthly[index] = input.outcome === 'cancel'
