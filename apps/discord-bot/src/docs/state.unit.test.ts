...
describe("docs state", () => {
  it("persists content-free provenance and expires it after 24 hours", async () => {
    ...
    try {
      ...
      await Effect.runPromise(store.record({
        provenance: {
          correlation: "a".repeat(64), atMillis: 2_000_000, corpusDigest: "sha256:test",
          engineConfiguration: "luna", sourceCount: 1, inputTokens: 2, outputTokens: 3, estimatedCostUsdMicros: 4,
        },
        quota: {
          principal: "b".repeat(64), atMillis: 2_000_000, inputTokens: 2, outputTokens: 3, costUsdMicros: 4,
        },
      }))
  ...
  it("serializes concurrent reservations and settles conservatively", async () => {
    ...
    try {
      ...
      const reserved = results.filter((result): result is { readonly _tag: "Reserved"; readonly id: string } => result._tag === "Reserved")
  ...
  it("fails closed on corrupted state and survives restart/month boundaries", async () => {
    ...
    try {
      ...
      const reservation = await Effect.runPromise(store.reserveMonthly({
        atMillis: Date.UTC(2026, 1, 1), costUsdMicros: 7, ceilingUsdMicros: 7,
      }))
