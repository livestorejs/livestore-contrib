  ...
  it.effect('plans an exact adoption with deterministic receipts and zero journal writes', () =>
    withJournal((journal) =>
      Effect.gen(function* () {
        ...
        const run = makeThreadReconciliationWorkflow(
          journal,
          observer({
...
const one = (sourceMessageId: typeof firstId, mode: ReconciliationMode, now: number) => ({
  selection: { _tag: 'One' as const, sourceMessageId },
  mode,
  now,
})
