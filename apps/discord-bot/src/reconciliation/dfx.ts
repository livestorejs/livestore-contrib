...
export const makeDfxThreadObservation = (rest: DfxThreadReader): ThreadObservationPort => ({
  observeSourceThread: input => rest.getChannel(input.sourceMessageId).pipe(
    Effect.match({
      onFailure: (cause): ThreadObservation => discordStatus(cause) === 404
        ? { _tag: "Absent" }
        : { _tag: "Unrun", reason: "discord_read_unavailable" },
...
export const classifyThreadChannel = (
  ...
  if (
    ...
    "id" in channel && channel.id === input.sourceMessageId &&
    "parent_id" in channel && channel.parent_id === input.channelId &&
    "type" in channel && (channel.type === 10 || channel.type === 11 || channel.type === 12)
  ) {
    return {
