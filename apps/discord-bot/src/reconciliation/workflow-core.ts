 ...
 */
/** The receipt digest is host-injected: node binds node:crypto sha256, the
 * Cloudflare worker a portable synchronous hash. Receipt ids are per-run
 * correlation labels, not durable cross-host identifiers. */
export const makeThreadReconciliationWorkflowCore =
  (
    journal: ThreadActionJournalService,
    observer: ThreadObservationPort,
    options: { readonly receiptDigestHex: (material: string) => string },
  ) =>
    ...
    Effect.gen(function* () {
      const validated = yield* Effect.try({
        try: () => validateRequest(request),
          ...
          cause instanceof InvalidReconciliationRequest
            ? cause
      ...
      const receipts = yield* Effect.forEach(
        selected.records,
        (record) => reconcileRecord(journal, observer, validated, record, options.receiptDigestHex),
        { concurrency: 1 },
      )
...
> => {
  ...
  return journal.listRecoverable.pipe(
    Effect.map((records) => {
...
const reconcileRecord = (
  ...
  digestHex: (material: string) => string,
): Effect.Effect<ReconciliationReceipt, JournalWriteError> => {
  // Parameter-binding adapter keeps every call site below identical to the
  // node original while threading the host-injected digest through.
  const receipt = (
    ...
    buildReceipt(digestHex, sourceMessageId, beforeState, afterState, disposition, mutated, threadId, unrunReason)
    ...
    if (record.state === 'pending') {
      ...
      if (request.mode._tag === 'Plan') {
        return Effect.succeed(
          receipt(record.sourceMessageId, 'pending', 'manual_review', 'would_close_interrupted_pending', false),
        )
      }
      return journal
        .markManualReview({
        ...
        .pipe(
          Effect.map((updated) =>
            receipt(record.sourceMessageId, 'pending', updated.state, 'interrupted_pending_closed', true),
          ),
        )
    }
    ...
    .pipe(
      ...
      Effect.flatMap((observation) => {
        if (observation._tag === 'Unrun') {
          ...
          return Effect.succeed(
            receipt(record.sourceMessageId, record.state, record.state, 'unrun', false, undefined, observation.reason),
          )
        }
        if (observation._tag === 'ExactSourceThread') {
          if (request.mode._tag === 'Plan') {
            return Effect.succeed(
              receipt(record.sourceMessageId, record.state, 'created', 'would_adopt', false, observation.threadId),
            )
          }
          return journal
            .markCreated({
            ...
            .pipe(
              Effect.map((updated) =>
                receipt(record.sourceMessageId, record.state, updated.state, 'adopted', true, observation.threadId),
              ),
            )
        }

        const deadlineElapsed = request.now >= record.reconcileBy
        const targetDisposition: ReconciliationDisposition =
          deadlineElapsed === true
            ? request.mode._tag === 'Plan'
              ? 'would_mark_manual_review'
              : 'manual_review'
            : request.mode._tag === 'Plan'
        ...
        if (request.mode._tag === 'Plan') {
          return Effect.succeed(receipt(record.sourceMessageId, record.state, targetState, targetDisposition, false))
        }
        ...
        return journal
          ...
          .pipe(
            Effect.map((updated) =>
              receipt(record.sourceMessageId, record.state, updated.state, targetDisposition, true),
            ),
          )
      }),
    )
...
const validateRequest = (request: ReconciliationRequest): ReconciliationRequest => {
  ...
  if (
    request.selection._tag === 'All' &&
    request.selection.limit !== undefined &&
    (Number.isSafeInteger(request.selection.limit) === false ||
      request.selection.limit < 1 ||
...
const buildReceipt = (
  ...
  const receiptId = `reconcile-${digestHex(material).slice(0, 20)}`
  return {
    receiptId,
    sourceMessageId,
    beforeState,
