...
export const decodeDiscordSourceMessage = (
  expected: typeof DiscordMessageRef.Type,
  value: unknown,
): DiscordSourceMessageFacts => {
  if (!isRecord(value) || value.guild_id !== expected.guildId || value.channel_id !== expected.channelId || value.id !== expected.messageId) {
    throw new TypeError("Discord source message identity did not match the requested guild, channel, and message")
  }
  if (!isRecord(value.author) || typeof value.author.id !== "string") {
  ...
  if (typeof value.content !== "string" || typeof value.type !== "number" || !Number.isInteger(value.type) || value.type < 0 || !Array.isArray(value.attachments)) {
  ...
  if (isReply && (!isRecord(reference) || typeof reference.message_id !== "string")) {
    throw new TypeError("Discord source message reply reference was invalid")
  }
  if (isReply && isRecord(reference)) Schema.decodeUnknownSync(DiscordSnowflake)(reference.message_id)
  const hasThread = value.thread !== undefined
  const thread = value.thread
  if (hasThread && (!isRecord(thread) || typeof thread.id !== "string")) {
    throw new TypeError("Discord source message existing thread was invalid")
  }
  const existingThreadId = hasThread
    ? Schema.decodeUnknownSync(DiscordSnowflake)(isRecord(thread) ? thread.id : undefined)
    : undefined
  const sourceChannelKind = isRecord(value.channel) && typeof value.channel.type === "number"
  ...
  return {
    ...
    stickerCount: Array.isArray(value.sticker_items) ? value.sticker_items.length : 0,
