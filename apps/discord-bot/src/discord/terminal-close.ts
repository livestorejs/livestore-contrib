...
export const terminalGatewayCloseCodes = [4004, 4010, 4011, 4012, 4013, 4014] as const
...
export const classifyGatewayClose = (code: number): GatewayCloseDisposition =>
  DiscordWS.isTerminalGatewayCloseCode(code) === true ? { _tag: 'Terminate', code } : { _tag: 'Reconnect', code }
 ...
 */
export const assessDfxTerminalCloseAdmission: Effect.Effect<void, TerminalCloseAdmissionError> = Effect.suspend(() => {
  const terminalSetIsExact = terminalGatewayCloseCodes.every(DiscordWS.isTerminalGatewayCloseCode)
  const transientRemainsRetryable = DiscordWS.isTerminalGatewayCloseCode(4000) === false
  const typedFailureIsPresent =
    new DiscordWS.TerminalGatewayCloseError({ code: 4004 })._tag === 'TerminalGatewayCloseError'
  ...
  return [LIB_VERSION === '1.0.15', terminalSetIsExact, transientRemainsRetryable, typedFailureIsPresent].every(
    (value) => value === true,
  )
    ? Effect.void
    : Effect.fail(
