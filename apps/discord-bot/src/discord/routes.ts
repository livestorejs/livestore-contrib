...
const decodeAutomaticMessage = (
  ...
  return Effect.try({
    try: () => decodeDiscordSourceMessage(expected, message),
    catch: (error) =>
...
const interactionRoute = (interaction: Discord.APIApplicationCommandInteraction) => ({
  interactionId: interaction.id,
  applicationId: interaction.application_id,
  token: Redacted.make(interaction.token),
})

const readStringOption = (options: Discord.APIApplicationCommandInteractionDataOption[] | undefined, name: string) => {
  const option = options?.find((candidate) => candidate.name === name)
  if (option === undefined) return undefined
  // oxlint-disable-next-line typescript-eslint/no-unsafe-enum-comparison
  return option.type === Discord.ApplicationCommandOptionType.STRING ? option.value : undefined
}
