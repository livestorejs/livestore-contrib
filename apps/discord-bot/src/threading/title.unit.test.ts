...
describe("thread titles", () => {
  ...
  it.effect("uses a valid proposal only for an explicitly disclosed channel", () =>
    Effect.gen(function* () {
      ...
      const title = yield* resolveThreadName(candidate, { aiTitleChannelIds: new Set([candidate.source.channelId]) }, {
  ...
  it.effect("falls back locally on provider or validation failure", () =>
    Effect.gen(function* () {
      const providerFailure = yield* resolveThreadName(candidate, { aiTitleChannelIds: new Set([candidate.source.channelId]) }, {
        propose: () => new TitleProposalError({ code: "quota", message: "quota exceeded" }),
      })
      const invalidProposal = yield* resolveThreadName(candidate, { aiTitleChannelIds: new Set([candidate.source.channelId]) }, {
