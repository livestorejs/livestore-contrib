import type { AutomaticRejectionReason, SourceChannelKind, ThreadCandidate } from './model.ts'
...
const greetingVocabulary = new Set(['hi', 'hello', 'hey', 'wave to say hi', 'welcome', 'good morning', 'good evening'])
...
const classifySource = (
  ...
  if (
    config.admittedParentKinds.has(candidate.sourceChannelKind) === false ||
    topLevelGuildKinds.has(candidate.sourceChannelKind) === false
  ) {
    return 'unsupported_channel_kind'
  }
  if (intentional === false && (candidate.messageKind === 'Reply' || candidate.hasMessageReference === true))
    return 'reply'
