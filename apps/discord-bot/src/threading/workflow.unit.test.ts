...
const makePorts = (
  ...
  reconciliation: {
    ...
    markCreating: (handle) => Effect.sync(() => { events.push(`creating:${handle.sourceMessageId}:${handle.claimToken}`) }),
    markCreated: (handle) => Effect.sync(() => { events.push(`created:${handle.sourceMessageId}:${handle.claimToken}`) }),
    markUnknownExternal: (handle) => Effect.sync(() => { events.push(`unknown_external:${handle.sourceMessageId}:${handle.claimToken}`) }),
    markFailed: (handle) => Effect.sync(() => { events.push(`failed:${handle.sourceMessageId}:${handle.claimToken}`) }),
  },
  mutation: {
...
describe("thread workflow", () => {
  ...
  it.effect("lets an authorized intentional trigger select a reply", () =>
    Effect.gen(function* () {
      const events: Array<string> = []
      const outcome = yield* makeThreadWorkflow(makePorts(events), config)({
        ...candidate,
        messageKind: "Reply",
