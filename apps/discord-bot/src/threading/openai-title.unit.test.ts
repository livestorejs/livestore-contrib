...
import {
  makeOpenAiThreadTitlePort,
  makeOpenAiTitleRequest,
  openAiTitleConfiguration,
} from "./openai-title.ts"
...
it.effect("distinguishes timeout from transport and rejects out-of-bound input before I/O", () =>
  Effect.gen(function* () {
    ...
    const transportClient = HttpClient.make(request => Effect.fail(new HttpClientError.HttpClientError({
