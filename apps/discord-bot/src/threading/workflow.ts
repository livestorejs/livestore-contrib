...
export interface ThreadReconciliationPort {
  ...
  readonly markCreated: (handle: ThreadClaimHandle, threadId: DiscordSnowflake) => Effect.Effect<void, ThreadReconciliationError>
  readonly markUnknownExternal: (handle: ThreadClaimHandle, code: string) => Effect.Effect<void, ThreadReconciliationError>
...
export const makeThreadWorkflow = (ports: ThreadWorkflowPorts, config: ThreadWorkflowConfig) =>
  Effect.fn("threading.requestThread")(function* (candidate: ThreadCandidate): Effect.fn.Return<ThreadOutcome> {
    ...
    if (created._tag === "Failure") {
      const record = created.failure.kind === "ambiguous"
