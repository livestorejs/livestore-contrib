...
describe("Discord bot source-executable black-box runtime", { timeout: 60_000 }, () => {
  it("crosses the source executable, health HTTP server, Unix control socket, CLI, and SQLite journal", async () => {
    ...
    try {
      ...
      expect(ready).toMatchObject({ apiVersion: 1, state: "ready", ready: true, capabilities: { threading: true }, restProbe: "ok", gateway: { state: "ready" } })
      ...
      expect(runtimeHealth.summary).toBe("state=ready ready=true authority=true journal=true gateway=ready rest=ok handlers=true docs=true")
      ...
      expect(rows).toEqual([
        { source_message_id: firstMessageId, channel_id: channelId, state: "created", trigger: "operator", thread_id: threadId(created.summary) },
        { source_message_id: concurrentMessageId, channel_id: channelId, state: "created", trigger: "operator", thread_id: threadId(concurrent[0]!.summary) },
      ])
...
) => await new Promise<{ readonly code: number | null; readonly stdout: string; readonly stderr: string }>((resolveChild, rejectChild) => {
  const child = spawnApplication(args, {
    cwd: appDirectory,
...
const makeTwoClientBarrier = async (path: string, upstreamPath: string) => {
  ...
  const server = createServer({ allowHalfOpen: true }, socket => {
    ...
    socket.on("data", chunk => { admitted.data += chunk })
    socket.once("end", () => {
      admitted.ended = true
      release()
    })
