...
*For: contributors to `adapter-node` · Assumes: the core runtime mental model
(leader ⇄ client-session) · Covers: how Node collapses that topology*
...
session, and that session simply *is* the leader — no election, no handover, a
...
process on the same store fights the first. Node is single-session *today* — a
