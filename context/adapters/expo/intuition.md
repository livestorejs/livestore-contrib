...
*For: contributors to `adapter-expo` · Assumes: the core runtime mental model
(leader ⇄ client-session) and the [Node adapter](../node/intuition.md) ·
Covers: how Expo differs from Node's already-collapsed topology*
...
is one JS thread, the leader lives in it, and the single session simply *is* the
...
The two things that actually make Expo *Expo*, rather than just a smaller Node:
...
1. **The database is `expo-sqlite`, not the WASM build.** Every other first-party
   ...
   the core leans on for determinism is now an *assumption* here, not a fact — see
   [.delta/DELTA-002](./.delta/DELTA-002-native-sqlite-substrate.md).
2. **Devtools point the other way.** Node *hosts* a devtools server the browser
   connects into. Expo *connects out* to a server on the developer's machine,
...
multi-tab web case; it is not — Expo is single-session *today* (a same-thread
