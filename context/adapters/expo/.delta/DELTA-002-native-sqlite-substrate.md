...
`src/make-sqlite-db.ts:37`, `:51`), so the _whole_ adapter is off the WASM build.
...
cross-platform materialization identity a _construction guarantee_. Under this
delta it becomes an _assumption_: identity between Expo and the WASM-based
