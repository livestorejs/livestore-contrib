...
This repository hosts realization intent for contrib-owned packages and the
intent of independently deployed applications owned by the contrib project.
The product intent layer — vision and product-wide requirements — lives in
`livestorejs/livestore` under `context/`; nothing here restates it.
...
Application nodes are different: they automate or operate a product surface
rather than implementing a pluggable runtime contract. They still reference
the core contract they serve, but own their application-specific requirements,
ontology, runtime, and operations here.
